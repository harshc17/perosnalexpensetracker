...
class ExpenseTracker:
    ...
    def list_expenses(self):
        print("\n--- Listed Expenses ---")
        # First, get memory expenses (latest added first) + non-duplicate main expenses
        memory_first = self.memory + [e for e in self.expenses if not any(self.is_duplicate(e, c) for c in self.memory)]
    ...
    def show_list_info(self):
        ...
        if self.expenses:
            for i, e in enumerate(self.expenses[:5]):  # Show first 5
                dt = datetime.fromtimestamp(e.date)
                print(f"  {i+1}. {e.description} - {e.amount:.2f} ({dt.strftime('%d-%m-%Y')})")
            if len(self.expenses) > 5:
                print(f"  ... and {len(self.expenses) - 5} more")
        else:
            print("  No expenses in main list")
            
        print("\nMemory List (Recent):")
        if self.memory:
            for i, e in enumerate(self.memory[:5]):  # Show first 5
                dt = datetime.fromtimestamp(e.date)
                print(f"  {i+1}. {e.description} - {e.amount:.2f} ({dt.strftime('%d-%m-%Y')})")
            if len(self.memory) > 5:
                print(f"  ... and {len(self.memory) - 5} more")
        else:
