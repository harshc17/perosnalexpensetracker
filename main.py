...
class ExpenseTracker:
    ...
    def edit_expense(self, idx, is_memory):
        ...
        dt = datetime.utcfromtimestamp(exp.date)
    ...
    def list_expenses(self):
        ...
        for i, e in enumerate(all_exp[:20]):
            dt = datetime.utcfromtimestamp(e.date)
    ...
    def category_summary(self):
        ...
        month_exp = [e for e in all_exp if datetime.utcfromtimestamp(e.date).month == month and datetime.utcfromtimestamp(e.date).year == year]
        if not month_exp:
    ...
    def find_highest_expense(self):
        ...
        dt = datetime.utcfromtimestamp(highest.date)
    ...
    def export_to_excel(self, year, month):
        ...
        # Highest expense
        highest = max(month_exp, key=lambda e: e.amount)
        highest_dt = datetime.utcfromtimestamp(highest.date)
        # --- Write to CSV ---
    ...
    def export_to_excel_date_range(self, start_date, end_date):
        ...
        os.makedirs(export_dir, exist_ok=True)
        all_exp = self.expenses + [e for e in self.memory if not any(self.is_duplicate(e, c) for c in self.expenses)]
        range_exp = [e for e in all_exp if start_date <= datetime.utcfromtimestamp(e.date) <= end_date]
        if not range_exp:
    ...
    def show_balance_history(self, is_bank):
        ...
        for i, h in enumerate(hist):
            dt = datetime.utcfromtimestamp(h.date)
            ...
            while True:
                ...
                if ch == 1:
                    try:
                        ...
                        if use_current == 'n':
                            # For custom date, set to noon to avoid day boundary issues
                            dt = dt.replace(hour=12, minute=0, second=0, microsecond=0)
                        else:
                            # For current date, use current time but ensure it's not near midnight
